  ...
  <div>
    <label for="toggle3" class="flex cursor-pointer select-none items-center">
      <div class="relative">
        ...
        />
        <div class="block h-8 w-14 rounded-full bg-meta-9"></div>
        <div
          :class="switcherToggle && '!right-1 !translate-x-full !bg-primary '"
          class="dot absolute left-1 top-1 flex h-6 w-6 items-center justify-center rounded-full bg-white transition"
        >
          <span :class="switcherToggle && '!block'" class="hidden text-white ">
            <svg
