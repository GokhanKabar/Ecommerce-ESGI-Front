        ...
        <div class="relative z-20 inline-block w-full">
          <div class="relative flex flex-col items-center">
            <div @click="open" class="w-full">
              <div
                class="mb-2 flex rounded border border-stroke py-2 pl-3 pr-3 outline-none transition focus:border-primary active:border-primaryt"
              >
                <div class="flex flex-auto flex-wrap gap-3">
                  <template v-for="index in selected" :key="index">
                    <div
                      class="my-1.5 flex items-center justify-center rounded border-[.5px] border-stroke bg-gray px-2.5 py-1.5 text-sm font-medium"
                    >
            ...
            <div class="w-full px-4">
              <div
                v-show="isOpen()"
                ref="target"
                class="max-h-select absolute left-0 top-full z-40 w-full overflow-y-auto rounded bg-white shadow"
              >
                <div class="flex w-full flex-col">
                  <div v-for="(option, index) in options" :key="index">
                    <div
                      @click="select(index, $event)"
                      class="w-full cursor-pointer rounded-t border-b border-stroke hover:bg-primary/5"
                    >
                      <div
                        :class="[
